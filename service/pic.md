...
图片处理服务仅在下载时触发，只有北京和上海地域支持像素10000*10000的图片处理请求，图片处理不支持range参数，加range参数会返回整个图片，目前部署图片处理服务的地区包括北京、上海、台北、香港、广州、迪拜、新加坡、胡志明、洛杉矶、拉各斯、首尔和伦敦，目前北京、上海、台北、新加坡、乌兰察布、迪拜、广州和伦敦支持所有功能，香港、胡志明、洛杉矶、拉各斯和首尔不支持索引切割、内切圆、圆角矩形、图片自适应方向、图片亮度、图片对比度、获取图片主色调、图片渐进显示、图片模糊和图片锐化等功能，乌兰察布不支持外网水印，需要使用需将图片上传对应地域US3,图片水印的imageurl时候使用内网域名url，若其他地域有图片处理需求，请联系技术支持。
